...
import { Calendar, MapPin, ArrowLeft, Car, Plane, Hotel, Crown, Check } from "lucide-react";
        ...
        <div className="container mx-auto px-4 -mt-20 relative z-10 pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div>
                {event.isOwn && (
                  <span className="inline-block px-2 py-1 rounded-md bg-success/15 text-success text-xs font-semibold mb-3">
                    ✓ No Fees
                  </span>
                )}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <button
                    onClick={() => setUberSelected(!uberSelected)}
                    className={`p-4 rounded-xl border text-left transition-all ${
                      uberSelected ? "border-primary bg-primary/10" : "border-border bg-card hover:border-primary/40"
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setHotelSelected(!hotelSelected)}
                    className={`p-4 rounded-xl border text-left transition-all ${
                      hotelSelected ? "border-primary bg-primary/10" : "border-border bg-card hover:border-primary/40"
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setFlightSelected(!flightSelected)}
                    className={`p-4 rounded-xl border text-left transition-all ${
                      flightSelected ? "border-primary bg-primary/10" : "border-border bg-card hover:border-primary/40"
                    }`}
                  >
              ...
              <div className="glass rounded-xl p-6 sticky top-24 space-y-5">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{quantity}x {tier.label}</span>
                    <span>${ticketPrice * quantity}</span>
                  </div>
                  {uberSelected && (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Uber Ride</span>
                      <span>$25</span>
                    </div>
                  )}
                  {hotelSelected && (
                  ...
                  <div className="flex justify-between text-sm text-success font-medium pt-1">
                ...
                <Button variant="hero" className="w-full animate-pulse-glow" size="lg">
                  Buy Tickets
                </Button>
                ...
                <div className="text-center">
                  <p className="text-xs text-muted-foreground flex items-center justify-center gap-1">
                    ...
                    <span>Members save on every purchase</span>
                  </p>
                </div>
              </div>
            </div>
