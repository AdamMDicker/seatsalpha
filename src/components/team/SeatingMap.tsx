...
const SeatingMap = ({ availableSections, selectedSection, setSelectedSection, game }: SeatingMapProps) => {
    ...
    <div>
      ...
      {mapZoomed && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setMapZoomed(false)}>
          ...
          <img src={rogersCentreMap} alt="Rogers Centre Seating Chart - Full Size" className="max-w-full max-h-[90vh] object-contain rounded-xl" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
