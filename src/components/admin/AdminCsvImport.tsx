...
const AdminCsvImport = () => {
  ...
  useEffect(() => {
    supabase.from("events").select("*").order("event_date").then(({ data }) => setEvents(data || []));
  }, []);
  ...
  const handleImport = async () => {
    ...
    for (const row of csvData) {
      try {
        // Check if this row has event info or just ticket info
        ...
        // Create ticket if section/price exists
        if (eventId && row.section && row.price) {
          ...
          success++; // Event created but no ticket data
        } else {
          errors++;
        }
      } catch {
